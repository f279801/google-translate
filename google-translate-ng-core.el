;;; google-translate-ng-core.el --- google-translate-ng core script -*- lexical-binding: t; -*-
...
;; This file is the core of `google-translate-ng' package.  It does contains
;; the most important and vital functions and variables for the
;; package functionality.
;;
;; The most important is the `google-translate-ng-request' function which
;; is intended to be used by other scripts and packages, expecially by
;; the packages which provides UI.
;;
;; This, `google-translate-ng-core', package doesn't provide any UI.
;;
;; `google-translate-ng-request' function sends http request to the
...
;;
;; `google-translate-ng-json-text-phonetic' - retrieves text phonetic;
;;
;; `google-translate-ng-json-translation' - retrieves translation;
;;
;; `google-translate-ng-json-translation-phonetic' - retrieves
;; phonetic translation;
;;
;; `google-translate-ng-json-detailed-translation' - retrieves
...
(require 'json)
(require 'url)
(require 'google-translate-ng-tk)

(defgroup google-translate-ng-core nil
  "Google Translate NG core script."
  :group 'processes)

(defvar google-translate-ng-base-url
  "http://translate.google.com/translate_a/single")

(defvar google-translate-ng-listen-url
  "http://translate.google.com/translate_tts")

(defun google-translate-ng--format-query-string (query-params)
...
(defun google-translate-ng--format-request-url (query-params)
  "Format QUERY-PARAMS as a Google Translate HTTP request URL.

QUERY-PARAMS must be an alist of field-value pairs."
  (concat google-translate-ng-base-url
          "?"
          (google-translate-ng--format-query-string query-params)))

(defun google-translate-ng--format-listen-url (query-params)
  "Format QUERY-PARAMS as an HTTP request URL for listen translation.

QUERY-PARAMS must be an alist of field-value pairs."
  (concat google-translate-ng-listen-url
          "?"
          (google-translate-ng--format-query-string query-params)))

(defun google-translate-ng-format-listen-url (text language)
  "Format listen url for TEXT and target LANGUAGE."
  (google-translate-ng--format-listen-url `(("ie"      . "UTF-8")
                                         ("q"       . ,text)
                                         ("tl"      . ,language)
                                         ("total"   . "1")
                                         ("idx"     . "0")
                                         ...
                                         ("client"  . "t")
                                         ("prev"    . "input")
                                         ("tk"      . ,(google-translate-ng--gen-tk text)))))

(defun google-translate-ng--http-response-body (url &optional for-test-purposes)
  "Retrieve URL and return the response body as a string.
Set FOR-TEST-PURPOSES if you want more debug information."
  (let ((google-translate-ng-backend-debug (or for-test-purposes
                                            google-translate-ng-backend-debug)))
    (with-temp-buffer
      (save-excursion
        (google-translate-ng-backend-retrieve url))
      (set-buffer-multibyte t)
      (buffer-string))))

(defun google-translate-ng--insert-nulls (string)
...
TARGET-LANGUAGE.  Returns response in json format."
  (let ((cleaned-text (google-translate-ng-prepare-text-for-request text)))
    (when (and
           (stringp cleaned-text)
           (> (length cleaned-text) 0))
      (json-read-from-string
       (google-translate-ng--insert-nulls
        (google-translate-ng--request source-language target-language text))))))

(defun google-translate-ng--request (source-language
                                  target-language
                                  text
...
TARGET-LANGUAGE."
  (google-translate-ng--http-response-body
   (google-translate-ng--format-request-url
    `(("client" . "t")
      ("ie"     . "UTF-8")
      ("oe"     . "UTF-8")
      ("sl"     . ,source-language)
...
(defun google-translate-ng-json-text-phonetic (json)
  "Retrieve text phonetic from the JSON.
The JSON is what returned by the `google-translate-ng-request' function."
  (mapconcat (lambda (item) (if (> (length item) 3) (aref item 3) ""))
             (aref json 0) ""))

(defun google-translate-ng-json-translation (json)
  "Retrieve the translation from the JSON.
The JSON is what returned by the `google-translate-ng-request' function."
  (mapconcat #'(lambda (item) (aref item 0))
             (aref json 0) ""))

(defun google-translate-ng-json-translation-phonetic (json)
  "Retrieve the translation phonetic from the JSON.
The JSON is what returned by the `google-translate-ng-request' function."
  (mapconcat #'(lambda (item) (if (> (length item) 2) (aref item 2) ""))
             (aref json 0) ""))

(defun google-translate-ng-json-detailed-translation (json)
...
(defun google-translate-ng-json-detailed-definition (json)
...
(defun google-translate-ng-json-suggestion (json)
...
(defun google-translate-ng-version ()
  "Query for google translate NG version."
  (interactive)
  (message "Google Translate NG (version): %s" "1.0.0"))


(provide 'google-translate-ng-core)

;;; google-translate-ng-core.el ends here
